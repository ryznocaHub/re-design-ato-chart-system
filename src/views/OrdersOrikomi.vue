<template>
  ...
  <h4 class="page-title">新聞折込　註文リスト</h4>
  <div class="content-wrapper">
    <div :class="device.mobile ? 'search-wrap-mobile' : 'search-wrap'" class="ms-3 me-3">
      ...
      <div>
        ...
        <input type="text" id="order_key" :class="device.mobile ? 'label-wide-mobile' : 'label-wide'" class="search-input" placeholder="入力 注文番号">
      </div>
      <div>
    ...
    <div v-for="(orderArray,j) in getOrders2d()" :key="j">
      <button class="order-cell" v-for="(order, i) in orderArray" @click="goOrderDetail(order.order_key)" :key="i">
        <div :class="device.mobile ? 'order-row-mobile' : 'order-row'">
          ...
          <div class="order-item">{{order.order_key}}</div>
          <div v-if="!device.mobile" class="order-item">{{allMenuStore.getters.findPostingStatusJName(order.status)}}</div>
          <div v-if="!device.mobile" class="order-item">{{order.extra.points[0]}}</div>
          <div v-if="!device.mobile" class="order-item">{{order.estimation.quantity.toLocaleString()}}</div>
          <div class="order-item">{{order.extra.upload_limit_date}}</div>
          <div class="order-item">{{order.insertion_date}}</div>
 ...
 /* async beforeMount(){
  ...
  methods: {
    ...
    async searchAgain(){
      ...
      const respData2d = await backendApi.refOrderLimitGroupByOrikomi(limit,Number(postingStatus),printOrderId,refText,fromTimeStr,toTimeStr)
      this.orderStore.commit("updateOrderArray2d", respData2d.value)

    },
    async goOrderDetail(orderKey){
      const resp =  await backendApi.refOrderOrikomi(orderKey)
      this.orderStore.commit("updateCurrentOrder",resp.value)
      location.href = "/order_orikomi/" + orderKey
    },
